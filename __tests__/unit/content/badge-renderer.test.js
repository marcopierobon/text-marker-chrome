...
describe('BadgeRenderer', () => {
  ...
  describe('constructor', () => {
    test('initializes with empty WeakSet', () => {
      expect(renderer.markedElements).toBeDefined();
    });
  });
  ...
  describe('createIcon', () => {
    test('creates img element for valid URL', () => {
      const icon = renderer.createIcon('https://example.com/icon.png', 'Test', '#ff0000');
    ...
    test('creates fallback for invalid URL', () => {
      const icon = renderer.createIcon('http://example.com/icon.png', 'Test', '#ff0000');
    ...
    test('handles multiple image load failures', () => {
      ...
      const icon1 = renderer.createIcon('https://example.com/icon1.png', 'First', '#ff0000');
      const icon2 = renderer.createIcon('https://example.com/icon2.png', 'Second', '#00ff00');
  ...
  describe('createFallbackIcon', () => {
    test('creates colored circle with initial', () => {
      ...
      expect(fallback.tagName).toBe('DIV');
      // jsdom converts hex to rgb format
      expect(fallback.style.backgroundColor).toMatch(/(#ff0000|rgb\(255, 0, 0\))/);
      expect(fallback.style.borderRadius).toBe('50%');
      expect(fallback.textContent).toBe('T');
    });
  ...
  describe('isMarked', () => {
    test('returns false for unmarked element', () => {
      ...
      expect(renderer.isMarked(element)).toBe(false);
    });
    ...
    test('returns true after marking element', () => {
      ...
      renderer.markedElements.add(element);
      expect(renderer.isMarked(element)).toBe(true);
    });
  });
  ...
  describe('clearBadges', () => {
    test('removes all badges from page', () => {
      ...
      global.document.querySelectorAll = () => mockBadges;
  ...
  describe('attachBadge', () => {
    test('attaches badge after text node parent', () => {
      ...
      const badge = document.createElement('span');
      badge.className = 'fool-badge';

      // Should not throw
      expect(() => renderer.attachBadge(textNode, badge)).not.toThrow();
    });
    ...
    test('returns false if element already marked', () => {
      ...
      renderer.attachBadge(textNode, badge1);
      const result = renderer.attachBadge(textNode, badge2);

      expect(result).toBe(false);
    });

    test('handles errors gracefully', () => {
      const result = renderer.attachBadge(null, document.createElement('span'));
      expect(result).toBe(false);
    });
    ...
    test('handles text node without parent', () => {
      ...
      const result = renderer.attachBadge(textNode, badge);
      expect(result).toBe(false);
    });
  });

  describe('clearBadges', () => {
    test('removes all fool-badge elements', () => {
      // Add badges using innerHTML
      document.body.innerHTML = `
        <span class="fool-badge">1</span>
        <span class="fool-badge">2</span>
        <div>other</div>
      `;

      renderer.clearBadges();

      const badgesAfter = document.querySelectorAll('.fool-badge');
