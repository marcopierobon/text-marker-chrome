...
describe('User Interactions - Unit Tests', () => {
  ...
  describe('Badge Click Interactions', () => {
    ...
    test('clicking on category URL button opens new tab', () => {
      ...
      const urlButton = Array.from(buttons).find(btn => btn.textContent.includes('🔗'));
      
      if (urlButton) {
        urlButton.click();
        expect(window.open).toHaveBeenCalledWith('https://example.com/category1', '_blank');
      }
    ...
    test('clicking on group URL button opens new tab', () => {
      ...
      const buttons = tooltip.querySelectorAll('button');
      const groupUrlButton = Array.from(buttons).find(btn => 
        btn.textContent.includes('Test Group') && btn.onclick
      );
