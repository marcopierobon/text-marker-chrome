...
import { isValidImageUrl, sanitizeUrl } from "../../../utils/url-validator";
...
describe("url-validator", () => {
  describe("isValidImageUrl", () => {
    ...
    test("accepts URLs from trusted domains without extensions", () => {
      expect(isValidImageUrl("https://lh3.googleusercontent.com/abc123")).toBe(
        true,
      );
      expect(
        isValidImageUrl(
    ...
    test("handles edge cases", () => {
      ...
      expect(isValidImageUrl("https://example.com/path/to/image.png")).toBe(
        true,
      ); // Nested path
      expect(isValidImageUrl("https://example.com/image.png?query=param")).toBe(
        true,
      ); // Query params
    });
  });
