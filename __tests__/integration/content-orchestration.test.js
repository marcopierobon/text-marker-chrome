...
describe('Content Script Orchestration - Advanced Tests', () => {
  ...
  describe('URL Filtering Complex Scenarios', () => {
    ...
    test('exact URL matches vs domain matches', () => {
      ...
      const tests = [
        { url: 'https://example.com/exact', domain: 'example.com', exactMatch: true, domainMatch: true },
        { url: 'https://example.com/other', domain: 'example.com', exactMatch: false, domainMatch: true },
        { url: 'https://sub.example.com/page', domain: 'sub.example.com', exactMatch: false, domainMatch: true },
        { url: 'https://other.com/page', domain: 'other.com', exactMatch: false, domainMatch: false }
      ];

      tests.forEach(({ url, domain, exactMatch, domainMatch }) => {
        const exactMatches = config.urlFilters.patterns.some(({ pattern, type }) => {
          if (type === 'exact') return url === pattern;
          return false;
        });

        const domainMatches = config.urlFilters.patterns.some(({ pattern, type }) => {
          if (type === 'domain') {
    ...
    test('blacklist mode with multiple patterns', () => {
      ...
      tests.forEach(({ domain, shouldRun }) => {
        ...
        const result = config.urlFilters.mode === 'blacklist' ? !matches : matches;
    ...
    test('switching between whitelist and blacklist modes', () => {
      ...
      let shouldRun = 'whitelist' === 'whitelist' ? matches : !matches;
      expect(shouldRun).toBe(true);

      // Blacklist mode - should NOT run on example.com
      shouldRun = 'blacklist' === 'whitelist' ? matches : !matches;
      ...
      shouldRun = 'whitelist' === 'whitelist' ? otherMatches : !otherMatches;
      expect(shouldRun).toBe(false);

      // Blacklist mode - should run on other.com
      shouldRun = 'blacklist' === 'whitelist' ? otherMatches : !otherMatches;
      expect(shouldRun).toBe(true);
    });
  });
  ...
  describe('Badge Lifecycle', () => {
    test('badge removal when element is removed from DOM', () => {
      document.body.innerHTML = '<div id="container"><span id="symbol">AAPL</span></div>';
      ...
      const initialBadges = document.querySelectorAll('.fool-badge').length;
      ...
      const remainingBadges = document.querySelectorAll('.fool-badge').length;
    ...
    test('badge persistence when element is moved in DOM', () => {
      ...
      const badgesAfterMove = document.querySelectorAll('.fool-badge').length;
  ...
  describe('Multi-Group Symbol Scenarios', () => {
    ...
    test('symbol with mix of categories (some with URLs, some without)', () => {
      ...
      const hasUrl = groups[0].categories.some(c => typeof c === 'object' && c.url);
