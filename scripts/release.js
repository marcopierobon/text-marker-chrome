...
if (!versionRegex.test(newVersion)) {
  console.error('Invalid version format. Use semantic versioning (e.g., 1.0.0, 1.0.1-beta)');
  process.exit(1);
}
...
try {
  execSync(`git add package.json manifest.json manifest.firefox.json`, { stdio: 'inherit' });
  execSync(`git commit -S -m "chore: bump version to ${newVersion}"`, { stdio: 'inherit' });
